...
import scipy
import spglib
from ase import Atoms, neighborlist, units
...
from ipywidgets import (Box, Button, Checkbox, ColorPicker, Dropdown,
                        ...
                        Label, Layout, Output, Select, Text, Textarea, VBox,
                        widgets)
...
from pfcc_extras.visualize.ngl_utils import (_get_standard_pos, add_axes_shape,
                                             add_force_shape, get_struct,
...
class JSMEBox(object):
    def __init__(self, set_func=print):
        """JSMEBox can create SMILES from JSME drawings  ."""
        # for own matlantis url
        jsme_url = os.environ["MATLANTIS_NOTEBOOK_URL"] + "files"
        ...
        if not os.path.exists(jsme_filepath):
            jsme_filepath_list = glob.glob(
                jsme_folderpath + "/**/jsme.nocache.js", recursive=True
            )
            if len(jsme_filepath_list) == 0:
                print(jsme_folderpath)
                print(jsme_url)
                raise FileNotFoundError(
                    "jsme.nocache.js not found, please prepare JSME software."
                )
            jsme_filepath = os.path.abspath(jsme_filepath_list[0])

        jsme_url += jsme_filepath.replace("home/jovyan", "")
        # 2023/10/23 いままで上でも動いていたので互換性を持たせるためここで修正
        jsme_url = re.sub(r"(?<!:)//+", "/", jsme_url)
        ...
        jsme_matlantis_htm = jsme_matlantis_template_htm.replace(
            "own_jsme_url", jsme_url
        ).replace("myid", nowtime)
        ...
        get_conformers_b = Button(
            layout={"width": "200px"}, description="get conformers"
        )
...
def get_conformers(smiles, rmsd_threshold=0.2, n_confs=50):
    ...
    try:
        ...
        autode.config.Config.rmsd_threshold = (
            rmsd_threshold + (ade_mol.n_atoms - 15) / 100
        )  # 時間短縮のため原子数に応じ閾値増加
...
class Nglbox:
    ...
    def __init__(self, calculator=None, atoms=None, w=460, h=340):
        ...
        self.check_index = Checkbox(
            description="index", indent=False, layout={"width": "80px"}
        )
        self.check_pbc = Checkbox(
            description="pbc", indent=False, layout={"width": "80px"}
        )
        self.check_axes = Checkbox(
            description="axes", indent=False, layout={"width": "80px"}
        )
        self.check_force = Checkbox(
            description="force", indent=False, layout={"width": "80px"}
        )
        self.check_chage = Checkbox(
            description="chage", indent=False, layout={"width": "80px"}
        )
        self.check_calc = Checkbox(
            description="calc", indent=False, layout={"width": "270px"}
        )
        self.dEbase = FloatText(
            description="base",
            value=0.0,
            layout={"width": "110px"},
        ...
        frontview.on_click(
            lambda change: self.view.control.rotate([0, 1 / 2**0.5, 1 / 2**0.5, 0])
        )
        sideview = Button(description="sideview", layout={"width": "140px"})
        sideview.on_click(
            lambda change: self.view.control.rotate([-0.5, 0.5, 0.5, 0.5])
        )
        ...
        self.viewwithcheck = VBox(
            [self.view, self.checkboxes, self.calcHbox, viewsetbox]
        )
    ...
    def show_force(self, change=""):
        if self.check_force.value:
            ...
            self.force_shape_id = add_force_shape(
                atoms, self.view, force_scale, [1, 0, 0]
            )
        else:
    ...
    def show_calc(self, change=""):
        if self.check_calc.value:
            ...
            self.check_calc.description = (
                f"E: {e:.4f} eV  fmax: {get_fmax(atoms):.4f}  dE: {e-basee:.3f}"
            )
            self.calcHbox.children = [self.check_calc, self.dEbase, self.setebutton]
    ...
    def show_charge_label(
        self, change="", threshold: float = 0.03, radius: float = 1.2
    ):
...
class Crystalsearch_mini:
    ...
    def __init__(self, set_func=print):
        ...
        self.anyonlybox = widgets.Dropdown(
            description="ele option",
            options=["within", "any", "only"],
            layout=inputlayout,
        )
        self.hullbox = widgets.BoundedFloatText(
            value=0.3, min=0, step=0.05, description="hullabove<", layout=inputlayout
        )
        self.dfoutput = Output(
            layout=Layout(
                overflow_y="hidden", overflow_x="hidden", height="360px", width="385px"
            )
        )
        ...
        self.rowslider = widgets.IntSlider(
            value=0, min=0, max=3000, step=1, description="No"
        )
    ...
    def search(self, change):
        ...
        if len(elements) > 0:
            cands = select_element(
                df=self.df, elements=elements, eleselect=self.anyonlybox.value
            )
    ...
    def showdf(self, change):
        ...
        with self.dfoutput:
            display(
                self.cands[self.showcol].iloc[
                    self.rowslider.value : self.rowslider.value + 7
                ]
            )
...
def select_element(df: pd.DataFrame, elements=["Fe"], eleselect="only") -> pd.DataFrame:
    """Select element

    Args:
        df: 抽出するデータフレーム
        elements: 抽出する元素。リスト形式
        eleselect: (only, any, within) onlyはelementsに含まれる元素のみ。anyはelementsに含まれる元素を含むレコード全部,withinは対象元素のみで構成する構造全部

    Returns:
        select_df (DataFrame): selected dataframe
    """
    if eleselect == "only":
        bool_list = [set(ele) == set(elements) for ele in df["elements"]]  # ['C', 'H']
    elif eleselect == "any":
        bool_list = [
            set(ele) == set(elements) | set(ele) for ele in df["elements"]
        ]  # ['C', 'H']
    elif eleselect == "within":
        bool_list = [
            np.min([ele in set(elements) for ele in eles]) > 0
            for eles in df["elements"]
        ]
...
def prim_to_conv(primitiveatoms):
    ...
    conventionalatoms = ase.Atoms(
        numbers=numbers, cell=lattice, scaled_positions=scaled_positions, pbc=True
    )
...
class File_seceltor:
    ...
    def __init__(self, set_func=None, maxlen=500):
        ...
        self.file_list_box = Select(
            options=[""], layout={"height": "400px", "width": "380px"}
        )
        self.file_list_box.observe(self.on_file_selected, names="value")
        self.inpath_box = Textarea(
            value="", placeholder="path", layout={"width": "300px"}
        )
        ...
        self.view = VBox(
            [HBox([self.inpath_box, up_button, h_button]), self.file_list_box]
        )
    ...
    def inpath_box_change(self, change):
        ...
        file_list = ["--- atoms file" + "-" * 30] + [
            os.path.basename(f) for f in file_list
        ]
        if len(file_list) > self.maxlen:  # max show files
            file_list = ["Displayed limit is" + str(self.maxlen)] + file_list[
                : self.maxlen
            ]
        file_list += ["--- dirs   ---" + "-" * 30] + [
            f.split("/")[-2] for f in glob.glob(path + "/*/")
        ]
        ...
        if change["new"] != change["old"]:
            ...
            if os.path.isfile(p):
                if os.path.getsize(p) > self.maxlen * 1e6:  # 500MByte
                    self.file_list_box.options = [
                        f"File size is over {self.maxlen}MB!"
                    ] + list(self.file_list_box.options)
                    return
                latoms = ase.io.read(p, index=":")
                if len(latoms) > self.maxlen:  # over 500 images
                    self.file_list_box.options = [
                        f"Traj limit!{len(latoms)} summarized to  {self.maxlen}"
                    ] + list(self.file_list_box.options)
...
class Cell_setter:
    def __init__(self, latoms=[Atoms("H")], set_func=print, undo_button=Button()):
        ...
        self.get_cellinfo_button = Button(
            description="get_cellinfo", layout={"width": "240px"}
        )
        self.out_box = Output(
            layout=Layout(
                overflow_y="scroll", overflow_x="hidden", height="220px", width="385px"
            )
        )
        self.cellinfo_str = Text(
            value="", description="cellpar", layout={"width": "385px"}
        )
        self.scale_atoms_check = Checkbox(
            description="scale atoms",
            Value=True,
            layout={"width": "240px"},
            style={"description_width": "100px"},
        )
        self.set_cell_from_cellpar_button = Button(
            description="set_cell_from_cellpar", layout={"width": "340px"}
        )
        self.scale = FloatText(
            min=0.5,
            max=2.0,
            value=1.01,
            step=0.001,
        ...
        self.set_cell_scale_button = Button(
            description="scale cell", layout={"width": "120px"}
        )
        self.view = VBox(
            [
                self.get_cellinfo_button,
                self.out_box,
        ...
        self.set_cell_from_cellpar_button.on_click(
            self.set_cell_from_cellpar_button_click
        )
    ...
    def get_cellinfo_button_click(self, b):
        with self.out_box:
            ...
            if not atoms.pbc.all():
                print(
                    "If you want to set pbc and cell, write cellpar values like 15,15,15,90,90,90 in the text box and press set_cell_from_cellpar button."
                )
    ...
    def set_cell_from_cellpar_button_click(self, b):
        ...
        if cellpar:
            atoms = get_newcell_atoms(
                atoms, cellpar, scale_atoms=self.scale_atoms_check.value
            )
            self.atoms = atoms
            self.set_func([atoms])
            self.get_cellinfo_button_click("")
    ...
    def set_cell_scale_button_click(self, b):
        ...
        if atoms.pbc.all():
            ...
            atoms = get_newcell_atoms(
                atoms, cell, scale_atoms=self.scale_atoms_check.value
            )
            self.atoms = atoms
            self.set_func([atoms])
            self.get_cellinfo_button_click("")
...
def get_cell_info(atoms):
    ...
    if atoms.pbc.all():
        ...
        print(
            f"a,b,c,α,γ,β = {  ','.join( map(lambda x: f'{x:.4f}', atoms.cell.cellpar())) }"
        )
...
class Indices_setter:
    def __init__(self, latoms=None):
        self.latoms = latoms
        ##### atom_indices  box  #####
        self.selected_atoms_textarea = Textarea(
            layout={"height": "40px", "width": "370px"}
        )
        self.xyz_box = Dropdown(
            options=["x", "y", "z", "i"], layout={"width": "50px"}, value="z"
        )
    ...
    def set_atoms(self, change):
        ...
        if self.xyz_box.value == "i":
            ...
            smols = [
                ...
                if sign * (atoms.positions[i, self.xyz_box.index] - value) >= 0
            ]
    ...
    def set_connected_atoms_text(self, change=[]):
        ...
        connected = [
            ...
            if component_list[i] == component_list[id]
        ]
...
class Mover:
    def __init__(self, latoms=[Atoms("H")], set_func=print, undo_button=Button()):
        ...
        self.xyz = Dropdown(
            options=["x", "y", "z"], description="direction", layout={"width": "150px"}
        )
        self.movev = FloatSlider(
            value=0,
            min=-5,
            max=5,
            step=0.1,
        ...
        self.disa0index = IntText(
            value=0,
            description="fix ",
            style={"description_width": "55px"},
            layout={"width": "140px"},
        )
        self.disa1index = IntText(
            value=0,
            description="move",
            style={"description_width": "55px"},
            layout={"width": "140px"},
        )
        self.distance = FloatSlider(
            min=0.7, max=6, step=0.1, description="distance", value=1.6
        )
    ...
    def move_button_click(self, b):
        ...
        if len(atom_indices) > 0:
            ...
            if self.xyz.value == "x":
                temp.positions += [self.movev.value, 0, 0]
            if self.xyz.value == "y":
                temp.positions += [0, self.movev.value, 0]
            if self.xyz.value == "z":
                temp.positions += [0, 0, self.movev.value]
...
class Surface_maker:
    def __init__(self, latoms=None, set_func=None, undo_button=None):
        ...
        self.min_slab_size = IntText(
            description="min_slab", value=15, layout={"width": "140px"}
        )
        self.min_vacuum = IntText(
            description="min_vacuum", value=30, layout={"width": "140px"}
        )
...
class Attach_maker:
    def __init__(
        self, latoms_list=None, set_func=None, undo_button=None, calculator=None
    ):
        self.latoms_list = latoms_list
        self.set_func = set_func
        self.calculator = calculator
...
def get_far_position(atoms, index, distance=3.0):
    ...
    nears_ind = np.argsort(
        atoms.get_distances(index, list(range(len(atoms))), mic=True)
    )[:50]
    ...
    for pos in cands:
        ...
        mindis += [
            min(checkatoms.get_distances(0, list(range(1, len(checkatoms))), mic=True))
        ]
...
def get_min_distance_ofaddatoms(atoms, reactindex0, reactindex1, lenoriginalatoms):
    ...
    nears_ind = np.argsort(
        baseatoms.get_distances(reactindex0, list(range(lenoriginalatoms)), mic=True)
    )[1:100]
    baseatoms = baseatoms[nears_ind]

    reactatoms = atoms[lenoriginalatoms:]
    nears_ind2 = np.argsort(
        reactatoms.get_distances(
            reactindex1 - len(atoms),
            list(range(len(atoms) - lenoriginalatoms)),
            mic=True,
        )
    )[:100]
    ...
    for pos in baseatoms.positions:
        ...
        mindis += [
            min(reactatoms.get_distances(0, list(range(1, len(reactatoms))), mic=True))
        ]

    return min(mindis)


def get_attach_atoms(
    atoms0, atoms1, a0i, a1i, distance=1.6, optsteps=0, calculator=None
):
    """
    ...
    for i in range(36):
        ...
        gmin += [
            get_min_distance_ofaddatoms(tempatoms, a0i, a1i + len(atoms0), len(atoms0))
        ]
    mid = np.argmax(gmin)
...
class Traj_setter:
    def __init__(
        ...
        insertBnn_afterAnn_button = Button(
            description="Insert B[##] after A[##]", layout={"width": "380px"}
        )
        insertBnn_afterAnn_button.on_click(self.insertBnn_afterAnn_button_click)
        insertallB_afterAnn_button = Button(
            description="Insert allB after A[##]", layout={"width": "380px"}
        )
        insertallB_afterAnn_button.on_click(self.insertallB_afterAnn_button_click)
        swap_button = Button(
            description="Swap A[##] and B[##]", layout={"width": "380px"}
        )
    ...
    def insertallB_afterAnn_button_click(self, b):
        ...
        with self.outbox:
            ...
            latoms_list[self.An.value] = (
                latomsA[:insertid] + latomsB + latomsA[insertid:]
            )
            self.reset_selector_view_func()
...
class Neb_maker:
    def __init__(
        ...
        self.set_func = set_func
        self.calculator = calculator
        self.initialstate0 = IntText(
            description="ini #",
            layout={"width": "130px"},
            style={"description_width": "50px"},
        )
        self.initialstate1 = IntText(
            description="##",
            layout={"width": "130px"},
            style={"description_width": "30px"},
        )
        self.finalstate0 = IntText(
            description="fin #",
            layout={"width": "130px"},
            style={"description_width": "50px"},
        )
        self.finalstate1 = IntText(
            description="##",
            layout={"width": "130px"},
            style={"description_width": "30px"},
        )
        self.nimages = IntText(
            description="nimages",
            value=15,
            step=5,
            layout={"width": "125px"},
        ...
        self.check_climb = Checkbox(
            description="climb",
            value=True,
            layout={"width": "125px"},
            style={"description_width": "50px"},
        )
        self.neb_k = FloatText(
            description="neb k",
            value=0.1,
            step=0.05,
            layout={"width": "125px"},
        ...
        self.re_neb_images = IntText(
            description="reNEB images", layout={"width": "150px"}
        )
        ...
        self.out_box = Output(
            layout=Layout(
                overflow_y="scroll", overflow_x="hidden", height="320px", width="385px"
            )
        )
    ...
    def run_neb(self):
        ...
        with self.out_box:
            ...
            if images[0].get_chemical_formula() != images[-1].get_chemical_formula():
                print(
                    "images dont have same formura,check re neb images index.",
...
def neb_calc(
    configs, climb=True, k=0.1, steps=200, fmax=0.05, calculator=None, logfile=None
):
...
def neb_plot(neblogfile, headrows=5, maxrows=9, h=260, w=360):
    ...
    for i in range(
        headrows, len(df) - 1, max(1, (len(df) - headrows) // (maxrows - headrows))
    ):
...
class NVT_maker:
    def __init__(self, latoms=None, set_func=None, calculator=None):
        ...
        self.time_step_box = FloatText(
            description="time_step fs", value=1, step=0.5, layout={"width": "180px"}
        )
        self.temperature_box = FloatText(
            description="temperature", value=473, step=10, layout={"width": "180px"}
        )
        self.num_md_steps_box = IntText(
            description="md_steps", value=1000, step=100, layout={"width": "180px"}
        )
        self.num_interval = IntText(
            description="interval", value=50, step=50, layout={"width": "180px"}
        )
        self.taut_box = FloatText(
            description="taut fs", value=1, step=0.5, layout={"width": "180px"}
        )
        self.output_filename = Text(
            description="path", value="./output/md/NVT_", layout={"width": "380px"}
        )
    ...
    def run_button_click(self, b):
        ...
        with self.out_box:
            ...
            MaxwellBoltzmannDistribution(
                atoms, temperature_K=temperature, force_temp=True
            )
            ...
            def print_dyn():
                ...
                print(
                    f"{imd: >3}  {epot:.3f}  {etot:.3f}  {temp_K:.1f}  {elapsed_time:.1f}"
                )
...
class Vib_maker:
    def __init__(self, latoms=None, set_func=None, calculator=None):
        ...
        self.set_func = set_func
        self.calculator = calculator
        self.delta_box = FloatText(
            description="delta", value=0.1, layout={"width": "180px"}
        )
        ...
        self.out_box = Output(
            layout=Layout(
                overflow_y="scroll", overflow_x="hidden", height="200px", width="385px"
            )
        )
        self.view = VBox(
            [
    ...
    def vib_readbutton_click(self, b):
        ...
        with self.out_box:
            ...
            if indices is not None:
                self.indices.selected_atoms_textarea.value = ", ".join(
                    map(str, indices)
                )
            self.vib = Vibrations(atoms, delta=delta, indices=indices, nfree=2)
            self.vdata = VibrationsData.from_2d(atoms, H, indices=indices)
    ...
    def remode_button_click(self, b):
        images = [
            i
            for i in self.vdata.iter_animated_mode(
                self.vib_mode.value,
...
class Thermo_maker:
    def __init__(self, latoms=None, set_func=None, calculator=None):
        ...
        self.set_func = set_func
        self.calculator = calculator
        self.temp_box = FloatText(
            description="temp_k", value=298, layout={"width": "160px"}
        )
        self.press_box = FloatText(
            description="P_kPa ", value=101.3, layout={"width": "160px"}
        )
        ...
        self.out_box = Output(
            layout=Layout(
                overflow_y="scroll", overflow_x="hidden", height="380px", width="385px"
            )
        )
        self.view = VBox(
            [
    ...
    def ideal_button_click(self, b):
        ...
        with self.out_box:
            ...
            if sum(np.iscomplex(vib_energies[6:])) > 0:
                print(
                    sum(np.iscomplex(vib_energies[6:])),
                    "Warn!Imaginary frequencies are found. change to 1e-4",
                )
                vib_energies[6:] = np.where(
                    np.iscomplex(vib_energies[6:]), 1e-4, vib_energies[6:]
                )
...
def my_2d_hess(atoms, delta=0.01, indices=None, thread_n=12, calculator=None):
    ...
    for i in range(thread_n):
        image = atoms.copy()
        estimator = Estimator(
            calc_mode=calc_mode, model_version=model_version
        )  # For Matlantis #202210
    ...
    while end_n < 6 * n:
        ...
        threads = [
            threading.Thread(
                target=run, args=(vibimages[i % thread_n], forces[i : i + 1])
            )
    ...
    for i in range(n):
        for dirc in [0, 1, 2]:
            H[i * 3 + dirc] = (
                0.25 * (forces[r] - forces[r + 1])[indices].ravel() / delta
            )
            r += 2
    H += H.copy().T
    del vibimages
...
class Opter:
    def __init__(self, latoms=None, set_func=None, undo_button=None, calculator=None):
        ...
        self.indices = Indices_setter(latoms=latoms)
        self.opt_cell_check = Checkbox(
            description="opt_cell", indent=False, layout={"width": "80px"}
        )
        ...
        self.fixbonda0 = IntText(
            description="a0",
            value=0,
            layout={"width": "110px"},
            style={"description_width": "25px"},
        )
        self.fixbonda1 = IntText(
            description="a1",
            value=0,
            layout={"width": "110px"},
            style={"description_width": "25px"},
        ...
        self.out_box = Output(
            layout=Layout(
                overflow_y="scroll", overflow_x="hidden", height="250px", width="385px"
            )
        )
        self.view = VBox(
            [
    ...
    def opt_button_click(self, b):
        ...
        with self.out_box:
            ...
            if self.fixbonda0.value != self.fixbonda1.value:
                ...
                print(
                    "fixbond",
                    self.fixbonda0.value,
                    self.fixbonda1.value,
                    "l=",
                    fixbond.initialize_bond_lengths(atoms)[0],
                )

            if self.opt_cell_check.value and sum(atoms.pbc) == 3:
                if self.maxstep_box.value >= 0.1:
                    self.maxstep_box.value = 0.005
                atoms = opt_cell_size(
                    atoms,
                ...
                if self.maxstep_box.value <= 0.005:
                    self.maxstep_box.value = 0.1
                atoms = myopt(
                    atoms,
                    fmax=self.fmax_box.value,
                    steps=self.steps_box.value,
...
def myopt(
    ...
    print(
        f"Epot: {atoms.get_potential_energy():.6f} fmax: {np.linalg.norm(atoms.get_forces(), axis=1, ord=2).max():.6f}"
    )
    if optimizer == "LBFGS":
...
def opt_cell_size(atoms, fmax=0.01, steps=200, maxstep=0.001, logfile=None):
    ...
    print(
        f"THICKNESS  {np.max (atoms.get_positions ()[:, 2]) - np.min (atoms.get_positions ()[:, 2]):.2f} A"
    )
...
class Liquid_maker:
    def __init__(
        ...
        self.atoms0b = Dropdown(
            options=range(20), description="Atoms0", layout={"width": "180px"}
        )
        self.atoms1b = Dropdown(
            options=range(20), description="Atoms1", layout={"width": "180px"}
        )
        ...
        self.out_box = Output(
            layout=Layout(
                overflow_y="scroll", overflow_x="hidden", height="300px", width="385px"
            )
        )
...
class Calc_setter:
    def __init__(self, calculator=None, ngl=None):
        ...
        l = Label("PFP setting")
        self.versionbox = widgets.Dropdown(
            description="version",
            options=versions,
            index=find_index(versions, calculator.estimator.model_version),
        )
        self.modebox = widgets.Dropdown(
            description="mode",
            options=modes,
            index=find_index(modes, calculator.estimator.calc_mode),
        )
        self.methodbox = widgets.Dropdown(
            description="method",
            options=methods,
            index=find_index(methods, calculator.estimator.method_type),
        )
        self.outbox = Output(layout={"width": "350px"})
        self.calculator = calculator
        ...
        self.view = VBox(
            [l, self.versionbox, self.modebox, self.methodbox, set_button, self.outbox],
            layout={"width": "370px"},
        )
        with self.outbox:
            print(
                "from  ",
                calculator.estimator.model_version,
                calculator.estimator.calc_mode,
                calculator.estimator.method_type.name,
            )
    ...
    def set_button_click(self, b):
        ...
        with self.outbox:
            print(
                "set to",
                self.calculator.estimator.model_version,
                self.calculator.estimator.calc_mode,
                self.calculator.estimator.method_type.name,
            )
...
def get_traj_energy_fmax_parallel(
    latoms,
    calculator=None,
    thread_n=12,
):
    ...
    def run(atoms, energy_fmax, calcn):
        ...
        energy_fmax[:] = [
            ...
            np.linalg.norm(atoms.get_forces(), axis=1, ord=2).max(),
        ]
    ...
    while end_n < n:
        ...
        threads = [
            threading.Thread(
                target=run, args=(latoms[i], energy_fmaxs[i : i + 1], i % thread_n)
            )
            for i in range(start_n, end_n, 1)
        ]
        for thread in threads:
...
class Matviewer:
    def __init__(self, calculator=None, load=True):
        ...
        ###  MENU ####
        self.add_menu = Dropdown(
            options=["ADD", "File", "SMILES or Draw", "Crystal", "MOF"],
            layout={"width": "140px"},
        )
        self.add_menu.observe(
            lambda change: self.add_function(change.new)
            if change["name"] == "value"
            else None,
            "value",
        )
        self.edit_menu = Dropdown(
            options=[
                "EDIT",
                "Move",
                "Utiledit",
        ...
        self.edit_menu.observe(
            lambda change: self.edit_function(change.new)
            if change["name"] == "value"
            else None,
            "value",
        )
        self.calc_menu = Dropdown(
            options=["CALC", "Setting", "OPT", "NEB", "VIB", "THERMO", "MD_NVT"],
            layout={"width": "140px"},
        )
        self.calc_menu.observe(
            lambda change: self.calc_function(change.new)
            if change["name"] == "value"
            else None,
            "value",
        )
        ...
        self.save_button = Button(
            description="Save", layout=Layout(height="30px", width="50px")
        )
        self.save_button.on_click(self.save_function)

        self.app_output = Output(
            layout={"height": "1px", "width": "1px"}
        )  # for back ground
        menubar = HBox(
            [
                self.add_menu,
        ...
        self.selector1 = IntSlider(
            value=1, min=0, max=0, step=1, layout=Layout(height="30px", width="170px")
        )
        self.selector1.observe(self.selector1_selected, names="value")
        self.del_n_button = Button(
            description="Del#", layout=Layout(height="30px", width="57px")
        )
        self.del_n_button.on_click(self.del_n_function)
        self.del_nn_button = Button(
            description="Del##", layout=Layout(height="30px", width="57px")
        )
        self.del_nn_button.on_click(self.del_nn_function)
        self.plot_button = Button(
            description="Plot", layout=Layout(height="30px", width="50px")
        )
        ...
        self.repeat_ctl = Repeater(
            set_func=self.set_func, latoms=self.latoms, undo_button=self.undo_button
        )
        self.cell_ctl = Cell_setter(
            set_func=self.set_func, latoms=self.latoms, undo_button=self.undo_button
        )
        self.move_ctl = Mover(
            set_func=self.set_func, latoms=self.latoms, undo_button=self.undo_button
        )
        self.util_ctl = Util_editor(
            set_func=self.set_func,
            latoms=self.latoms,
            latoms_list=self.latoms_list,
            undo_button=self.undo_button,
        )
        self.traj_ctl = Traj_setter(
            reset_selector_view_func=self.reset_selector_view,
            latoms_list=self.latoms_list,
        )

        self.sur_ctl = Surface_maker(
            set_func=self.set_func, latoms=self.latoms, undo_button=self.undo_button
        )
        self.opt_ctl = Opter(
            set_func=self.set_func,
        ...
        self.nvt_ctl = NVT_maker(
            set_func=self.set_func, latoms=self.latoms, calculator=self.calculator
        )

        self.attach_ctl = Attach_maker(
            set_func=self.set_func,
            latoms_list=self.latoms_list,
            undo_button=self.undo_button,
            calculator=self.calculator,
        )
        self.liquid_ctl = Liquid_maker(
            set_func=self.set_func,
            latoms_list=self.latoms_list,
            undo_button=self.undo_button,
            calculator=self.calculator,
        ...
        self.neb_ctl = Neb_maker(
            set_func=self.set_func,
            latoms_list=self.latoms_list,
            calculator=self.calculator,
        )
        self.vib_ctl = Vib_maker(
            set_func=self.set_func, latoms=self.latoms, calculator=self.calculator
        )
        self.thermo_ctl = Thermo_maker(
            set_func=self.set_func, latoms=self.latoms, calculator=self.calculator
        )
        ...
        self.set_ctl = VBox(
            [self.calcset.view, self.dispctl], layout={"width": "380px"}
        )
    ...
    def add_function(self, choice):
        if choice == "File":
            self.leftctlbox.children = [self.fileselector.view]
        if choice == "SMILES or Draw":
            self.leftctlbox.children = [self.jsme.view]
        if choice == "Crystal":
            self.leftctlbox.children = [self.crystalsearch.view]
        if choice == "MOF":
            self.leftctlbox.children = [self.mofsearch.view]
        self.calc_menu.index = 0
        self.edit_menu.index = 0

    def edit_function(self, choice):
        if choice == "Move":
            self.leftctlbox.children = [self.move_ctl.view]
        if choice == "Utiledit":
            self.leftctlbox.children = [self.util_ctl.view]
        if choice == "Repeat":
            self.leftctlbox.children = [self.repeat_ctl.view]
        if choice == "Cell":
            self.leftctlbox.children = [self.cell_ctl.view]
        if choice == "Surface":
            self.leftctlbox.children = [self.sur_ctl.view]
        if choice == "Attach":
            self.leftctlbox.children = [self.attach_ctl.view]
        if choice == "Liquid":
            self.leftctlbox.children = [self.liquid_ctl.view]
        if choice == "Traj":
            self.leftctlbox.children = [self.traj_ctl.view]
    ...
    def calc_function(self, choice):
        if choice == "Setting":
            self.leftctlbox.children = [self.set_ctl]
        if choice == "OPT":
            self.leftctlbox.children = [self.opt_ctl.view]
        if choice == "NEB":
            self.leftctlbox.children = [self.neb_ctl.view]
        if choice == "VIB":
            self.leftctlbox.children = [self.vib_ctl.view]
        if choice == "THERMO":
            self.leftctlbox.children = [self.thermo_ctl.view]
        if choice == "MD_NVT":
            self.leftctlbox.children = [self.nvt_ctl.view]
        self.add_menu.index = 0
        self.edit_menu.index = 0
    ...
    def save_function(self, change):
        ...
        traj_files = sorted(
            glob.glob(os.path.join(directory, "*.traj")),
    ...
    def set_selector_view(self):
        ...
        if s0len != len(self.selector0.options):
            self.selector0.options = list(range(s0len))
        s1max = len(self.latoms_list[self.selector0.index]) - 1
        if s1max != self.selector1.max:
            self.selector1.max = s1max
        self.selector0.observe(self.selector0_selected, names="value")
        self.selector1.observe(self.selector1_selected, names="value")
    ...
    def set_func(self, result_latoms, addnewno=False):
        if len(result_latoms) > 0:
            ...
            if len(result_latoms) > 1 or addnewno:
                ...
                self.latoms_list[self.selector0.value][
                    self.selector1.value
                ] = result_latoms[0]
    ...
    def display_set_button_click(self, b):
        ...
        with self.app_output:
            display(
                HTML(
                    f"<style> .widget-vbox {{background-color: {self.color_picker.value} ; padding: 2px }}</style>"
                )
            )
    ...
    def full_button_click(self, b):
        ...
        if self.wi.value < 1000:
            self.wi.value += 200
        if self.hi.value < 700:
            self.hi.value += 200
        self.display_set_button_click("")
